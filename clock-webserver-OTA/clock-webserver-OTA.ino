...
<div class="tab-content" id="tab-kontrol">
  ...
  <div class="btn-row">
    ...
    <button class="btn" id="btn_mode1" onclick="setMode(1)">animasyonu durdur</button>
  </div>
...
<div class="tab-content" id="tab-saat">
  ...
  <div class="alarm-section" id="alarm-section">
    ...
    <button class="btn full red" id="btn_alarm_stop" onclick="stopAlarmBtn()" style="display:none;margin-top:1px">
      alarmı durdur
    </button>
  </div>
...
function fetchStatus() {
  fetch('/api/status', {signal: AbortSignal.timeout(2000)})
    ...
    .then(d => {
      ...
      if (d.mode_num === 0) document.getElementById('btn_mode0').classList.add('active');
      ...
      document.getElementById('anim_hour').classList.remove('running');
      document.getElementById('anim_alarm').classList.remove('running');
      if (d.mode_num === 1) {
        document.getElementById('anim_hour').classList.add('running');
        animStop.style.display = 'block';
      } else if (d.mode_num === 2) {
        document.getElementById('anim_alarm').classList.add('running');
        animStop.style.display = 'block';
      } else {
        animStop.style.display = 'none';
      }

      // Alarm badge + durdur butonu
...
function api(url, successMsg, cb) {
  fetch(url, {signal: AbortSignal.timeout(3000)})
    .then(r => {
      if (!r.ok) toast('hata!');
      else { if (successMsg) toast(successMsg); if (cb) cb(); }
    })
...
function setMode(m) {
  ['mode0','mode1'].forEach(id => document.getElementById('btn_'+id).classList.remove('active'));
  document.getElementById('btn_mode'+m).classList.add('active');
  api('/api/mode?val='+m, m==0 ? 'normal moda donuldu' : 'durduruldu');
}
...
function triggerAnim(type) {
  document.getElementById('anim_hour').classList.remove('running');
  document.getElementById('anim_alarm').classList.remove('running');
  document.getElementById('anim_'+(type==='hour'?'hour':'alarm')).classList.add('running');
  document.getElementById('btn_anim_stop').style.display = 'block';
  api('/api/anim?type='+type,
      type==='hour' ? 'saat basi animasyonu' : 'alarm animasyonu',
      () => setTimeout(fetchStatus, 300));
}

function stopAnim() {
  api('/api/anim/stop', 'animasyon durduruldu', () => setTimeout(fetchStatus, 200));
}
...
function setAlarm(enable) {
  ...
  api('/api/alarm?h='+h+'&m='+m+'&en='+(enable?1:0),
    enable
      ? 'alarm: '+String(h).padStart(2,'0')+':'+String(m).padStart(2,'0')
      : 'alarm kapatildi',
...
void handleApiStatus() {
    ...
    String modeName;
    if      (currentMode == 0) modeName = "normal";
    else if (currentMode == 1) modeName = "saat basi";
    else                       modeName = "alarm";
...
void handleApiMode() {
    ...
    currentMode = m;
    if (m == 1) { animRunning=true; animStep=0; lastAnimUpdate=millis(); }
    else        { stopAlarmNow(); }
    server.send(200);
}
...
void loop() {
    ...
    // Öncelik: alarm > saat başı > normal
